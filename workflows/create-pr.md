...
arguments: [--base <branch>] [instructions]
trigger: Use when the user asks to open, create or draft a pull request for the current branch. Detects the base branch, writes an English ticket-based title and body, fills the repository PR template and pushes before creating the PR through the gh CLI.
requires: gh
---
...
```bash
{{command:create-pr}} [--base <branch>] [instructions]
```
...
{{command:create-pr}} --base develop

# Override the default language
{{command:create-pr}} write the body in French

# Add context to the body
{{command:create-pr}} mention that the migration must run before deploy
```
