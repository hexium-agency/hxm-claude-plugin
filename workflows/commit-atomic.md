...
arguments: [--dry-run]
trigger: Use when the user asks to split pending work into several atomic commits, or to commit a large working tree mixing unrelated changes. Groups changes by logical unit and creates one conventional commit per group after explicit confirmation.
requires: none
---
