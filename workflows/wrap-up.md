...
arguments: [ticket] [--base <branch>]
trigger: Use when the user asks to wrap up, summarise or review the work done on the current branch before opening a pull request. Retrieves the linked ClickUp ticket, analyses every change and reports on completeness and quality.
requires: clickup
---
...
```bash
{{command:wrap-up}} [ticket] [--base <branch>]
```
...
{{command:wrap-up}} abc123

# Specify a different base branch
{{command:wrap-up}} --base develop
```
...
<!-- /provider -->
<!-- provider:codex -->
If the diff is very large (more than 200 files changed), analyse the files in batches — start from the `--name-status` list and inspect the files group by group, rather than loading the entire diff at once.
