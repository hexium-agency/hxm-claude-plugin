---
name: fix-tests
description: Diagnose a failing Laravel/Pest test suite — classify each failure as infrastructure noise or a real branch regression, and fix only the regressions
arguments: [<pasted-test-output>] [--run]
trigger: Use when a Laravel or Pest test suite fails and the user asks to diagnose or repair it. Classifies every failure as infrastructure noise or as a real regression introduced by the branch, and fixes only the regressions.
requires: none
---
...
```bash
{{command:fix-tests}} [<pasted-test-output>] [--run]
```
