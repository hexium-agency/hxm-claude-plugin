...
arguments: <target> [--type inline|centralized] [--level brief|detailed]
trigger: Use when the user asks to document code, a feature, an API or a system architecture, or to write or refresh a README or technical documentation.
requires: none
---
...
```bash
{{command:document}} <target> [--type inline|centralized] [--level brief|detailed]
```
...
```bash
{{command:document}} src/auth/login.js --type inline --level detailed
...
```bash
{{command:document}} src/payment --type centralized --level brief
...
```bash
{{command:document}} . --type centralized --level detailed
