...
arguments: [--from <tag>] [--to <ref>] [--output <file>]
trigger: Use when the user asks for a changelog, release notes, or a summary of what shipped since a given tag. Groups conventional commits between two git refs by type and writes the result to stdout or to a file.
requires: none
---

# Changelog Generator

Generates a changelog by analyzing conventional commits since the last version tag. Complements `{{command:bump}}` by producing human-readable release notes.

## Usage

```bash
{{command:changelog}} [--from <tag>] [--to <ref>] [--output <file>]
```
...
{{command:changelog}} --from v1.2.0 --to v1.3.0

# Write changelog to file
{{command:changelog}} --output CHANGELOG.md

# Generate changelog since a specific tag to HEAD
{{command:changelog}} --from v2.0.0
```
...
- Pairs well with `{{command:bump}}`: run `{{command:changelog}} --output CHANGELOG.md` before bumping the version.
