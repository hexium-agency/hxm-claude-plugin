...
arguments: <target> [--audience developer|architect|junior] [--style technical|visual|tutorial]
trigger: Use when the user asks to explain, break down or walk through existing code, a pattern, a module or a system architecture, without modifying anything.
requires: none
---
...
```
{{command:explain}} <target> [--audience developer|architect|junior] [--style technical|visual|tutorial]
```
...
{{command:explain}} src/services/AuthService.ts

# Visual explanation of authentication flow for architects
{{command:explain}} "authentication flow" --audience architect --style visual

# Tutorial-style explanation for junior developers
{{command:explain}} middleware/validate.ts --audience junior --style tutorial

# Technical deep-dive into a specific function
{{command:explain}} handleUserLogin --style technical
```
